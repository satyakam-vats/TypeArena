import { BarChart3, Moon, Sun, Trophy, User, Volume2, VolumeX } from "lucide-react";
...
export function Header({ theme, onToggleTheme }: HeaderProps) {
  ...
  return <header className="mx-auto flex w-full max-w-6xl items-center justify-between px-5 py-6 sm:px-8">
    ...
    <nav className="hidden items-center gap-6 text-sm text-[var(--muted)] sm:flex">
      ...
      <NavLink to="/daily" className={({ isActive }) => isActive ? "text-[var(--ink)]" : "hover:text-[var(--ink)]"}>daily</NavLink>
      <NavLink to="/practice" className={({ isActive }) => isActive ? "text-[var(--ink)]" : "hover:text-[var(--ink)]"}>practice</NavLink>
